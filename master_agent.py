...
# ⚡ API KEY SETUP
load_dotenv()
gemini_api_key = 'AIzaSyAdVsdvPvDLXKaIz5c0UGo3Sp5oULcSjq4' 
...
def load_precautions_data():
    ...
    precautions_df = pd.read_csv('/Users/zhuoy1/Documents/machine_talents_hack/logo_1753104564000.csv')
...
async def complete_triage_workflow(text: str) -> Dict:
    """Complete end-to-end triage workflow"""
    symptoms_data = {'symptoms': extract_symptoms_from_text(text)}
...
async def test_complete_triage_system():
    ...
    test_cases = [
        {
            'input': "I'm having chest pain, difficulty breathing and I'm sweating",
...
async def text_triage_endpoint(
    symptoms: str = Form(...),
    additional_info: str = Form(""),
    age: Optional[int] = Form(None),
    gender: Optional[str] = Form(None)
):
    """Text-based triage endpoint for symptom analysis"""
    try:
        # Extract symptoms from text
        extracted_symptoms = extract_symptoms_from_text(symptoms)

        # Add any additional context
        if additional_info:
            additional_symptoms = extract_symptoms_from_text(additional_info)
            extracted_symptoms.extend(additional_symptoms)

        # Remove duplicates while preserving order
        unique_symptoms = list(dict.fromkeys(extracted_symptoms))

        # Run complete triage workflow
        result = await complete_triage_workflow(symptoms + " " + additional_info)
        ...
        return JSONResponse(content={
            ...
            "urgency_level": result['triage_assessment']['urgency_level'],
            "confidence": result['triage_assessment']['confidence'],
            "reasoning": result['triage_assessment']['reasoning'],
            "likely_conditions": result['condition_predictions']['likely_conditions'],
            "condition_confidence_scores": result['condition_predictions']['confidence_scores'],
            "specific_precautions": result['medical_response']['specific_precautions'],
            "general_advice": result['medical_response']['general_advice'],
            "when_to_seek_help": result['medical_response']['when_to_seek_help'],
            "timeframe": result['medical_response']['timeframe'],
            "voice_response": result['voice_response']['voice_text'],
            "patient_demographics": {
                "age": age,
                "gender": gender
            },
            "data_sources": result['data_sources'],
            "disclaimer": result['medical_response']['disclaimer']
        })
